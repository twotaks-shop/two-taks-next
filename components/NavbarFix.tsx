...
function ProfileIcon() {
	...
	return (
		<div className="relative" data-profile-dropdown>
			...
			{isDropdownOpen && isAuthenticated() && customer && (
...
function MobileAccountSection({ closeMenu }: { closeMenu: () => void }) {
	const { customer, isAuthenticated, logout } = useCustomerStore();
	const { openLogin } = useAuthModalStore();

	const handleAuthAction = () => {
		if (isAuthenticated()) {
			// Already authenticated, go to account
			closeMenu();
		} else {
			openLogin();
		}
	...
	if (isAuthenticated() && customer) {
		return (
			<div className="space-y-2">
				...
				<button
					onClick={handleLogout}
					className="flex items-center py-3 text-sm text-red-600 hover:text-red-700 font-league-spartan w-full text-left"
				>
					<svg
						className="mr-3"
						xmlns="http://www.w3.org/2000/svg"
	...
	return (
		<>
			<button
				...
				className="flex items-center py-3 text-sm text-neutral-600 hover:text-black font-league-spartan w-full text-left"
			>
				<svg
					className="mr-3"
					xmlns="http://www.w3.org/2000/svg"
				...
				<div className="fixed inset-0 top-20 bg-white z-50 overflow-y-auto pb-16">
					<div className="py-4">
						...
						<div className="mt-6 px-4">
							<MobileAccountSection closeMenu={closeMenu} />
						</div>
					</div>
				</div>
			)}
